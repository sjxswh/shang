				...
				<li v-for="data in dataList" :data="JSON.stringify(data)">
					...
					<div class="campaigns-content-main" :data="JSON.stringify(data)">
						<div class="campaigns-title" :data="JSON.stringify(data)">
							<a @touchstart="LinksHrf($event)" :data="JSON.stringify(data)">
								<span :data="JSON.stringify(data)">{{data["name"]}}</span>
							</a>
							<router-link to="/Offers" :data-cId="data.affiliateId" :data-name="data.affiliateName" @touchstart="cOffer($event)" class="flowOffer">
								<span class="iconfont icon-gengduo" :data-cId="data.affiliateId" :data-name="data.affiliateName"  @touchstart="cOffer($event)"></span>
							</router-link>
						</div>
						<a @touchstart="LinksHrf($event)" :data="JSON.stringify(data)">
...
<script>
	...
	export default{
		...
		methods: {
			...
			cOffer(ev){
				this.datas = {"affiliateName":ev.target.getAttribute("data-name"),"affiliateId":ev.target.getAttribute("data-cId")}
			...
			reduceDate(){
				...
				this.Data = commont.reduceDate(this.Date,this.date,this.$store.state.data.status,'affiliate')
